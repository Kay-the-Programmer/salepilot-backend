...
export const createCustomer = async (req: express.Request, res: express.Response) => {
    ...
    try {
        const result = await db.query(
            ...
            [id, name, email, phone, address, notes, createdAt, storeCredit || 0, 0]
        );
...
export const updateCustomer = async (req: express.Request, res: express.Response) => {
    ...
    try {
        const result = await db.query(
            'UPDATE customers SET name=$1, email=$2, phone=$3, address=$4, notes=$5, store_credit=$6, account_balance=$7 WHERE id=$8 RETURNING *',
            [name, email, phone, address, notes, storeCredit, accountBalance, id]
        );
        if (result.rowCount === 0) {
            return res.status(404).json({ message: 'Customer not found' });
        }
